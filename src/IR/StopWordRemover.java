...
//    private static Porterstemmer stemmer;
    ...
    public static List<String> removeStopWord(List<String> wordList) {
        ...
        wordList.removeAll(JAVA_KEYWORDS_STOP_WORD_SET);
    ...
    private static boolean isJavaKeyWord(String word) {
        boolean isKeyword = JAVA_KEYWORDS_STOP_WORD_SET.contains(word.toLowerCase());

        return isKeyword;
    }

    private static boolean isEnglishStopword(String word) {
        boolean isEnglishStopword = ENG_STOP_WORDS_SET.contains(word.toLowerCase());
    ...
    static {
        String[] keywords = { "abstract", "continue", "for", "new", "switch", "assert", "default", "goto", "package",
                ...
                "class", "finally", "long", "strictfp", "volatile", "const", "float", "native", "super", "while", "org",
                "eclipse", "swt", "string", "main", "args", "null", "this", "extends", "true", "false" };
        JAVA_KEYWORDS_STOP_WORD_SET = new TreeSet<String>();
        for (int i = 0; i < keywords.length; i++) {
            String word = keywords[i].trim().toLowerCase();
